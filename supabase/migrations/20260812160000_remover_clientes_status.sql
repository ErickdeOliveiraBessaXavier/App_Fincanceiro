-- ============================================================================
-- Remove clientes.status — coluna escrita e nunca lida
--
-- Ela tinha CHECK, índice e era gravada em três lugares (INSERT do app e as
-- duas RPCs de importação), sempre com 'ativo'. Nada a atualizava depois: nem
-- pagamento, nem acordo, nem exclusão. E `useClientes` DESCARTAVA o valor do
-- banco, recalculando a situação a partir dos títulos.
--
-- Resultado: um campo com aparência de fonte de verdade que nunca foi. A ficha
-- do cliente lia essa coluna direto e por isso exibia "Ativo" para todo mundo,
-- inclusive inadimplente — corrigido junto, passando a derivar como as demais
-- telas.
--
-- A situação do cliente vive em src/domain/clientes/situacao.ts, derivada dos
-- status consolidados dos títulos, com precedência
-- inadimplente > em_acordo > quitado > ativo.
-- ============================================================================

-- ============== 1. Importações param de gravar a coluna ==============
-- Recriadas a partir da definição que estava no banco, retirando apenas
-- `status` da lista de colunas e o 'ativo' do VALUES.

CREATE OR REPLACE FUNCTION public.importar_titulo(p_company_id uuid, p_cliente_nome text, p_cpf_cnpj text, p_valor numeric, p_vencimento date, p_contato text DEFAULT NULL::text, p_descricao text DEFAULT NULL::text, p_cobrador text DEFAULT NULL::text, p_vendedor text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_cliente_id IS NULL THEN
    INSERT INTO public.clientes (company_id, nome, cpf_cnpj, telefone, cobrador_id, vendedor_id, created_by)
    VALUES (v_company, trim(p_cliente_nome), v_cpf, NULLIF(trim(coalesce(p_contato,'')), ''), v_cobrador_id, v_vendedor_id, auth.uid())
...
BEGIN
  ...
  IF v_cliente_id IS NULL THEN
    ...
    VALUES (v_company, trim(p_cliente_nome), v_cpf, NULLIF(trim(coalesce(p_contato,'')),''), v_cidade, v_estado, v_cobrador_id, v_vendedor_id, auth.uid())
    RETURNING id INTO v_cliente_id;
  ELSE
...
-- ============== 2. Fim da coluna ==============
-- O índice existia para filtrar por um valor que era sempre o mesmo.
DROP INDEX IF EXISTS public.idx_clientes_company_status;

-- O CHECK clientes_status_check sai junto com a coluna.
